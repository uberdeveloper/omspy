...
def add_name(data, segment:Optional[str]="cash")->pd.DataFrame:
    ...
    if segment.lower() == "cash":
        ...
        data['inst_name'] = [f"{k}:{get_name_for_fno_symbol(a,str(b),c,convert_strike(d))}" for a,b,c,d,k in zip(data.instrumentname.values, data.expiry.values, data.optiontype.values,data.strike.values,data.exchange.values)]
        return data
    else:
        return data
